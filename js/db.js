/* =============================================
   DATABASE v6 - With Itinerary Support
   ============================================= */
...
const DB = {
    KEYS: {
        ...
        ITINERARIES: 'tripzar_itineraries',
        ITIN_COUNTER:'tripzar_itin_counter',
    ...
    init() {
        if (!localStorage.getItem(this.KEYS.SETTINGS)) {
            ...
            let updated = false;
            ['building','street','area','city','pincode','country',
             'invoice_type','llpin'].forEach(k => {
        ...
        const arrayKeys = [
            this.KEYS.CUSTOMERS, this.KEYS.INVOICES,
            this.KEYS.PURCHASES, this.KEYS.SUPPLIERS,
            this.KEYS.ITINERARIES
        ];
        ...
        if (!localStorage.getItem(this.KEYS.ITIN_COUNTER)) {
            localStorage.setItem(this.KEYS.ITIN_COUNTER, '0');
        }
        ...
        if (!localStorage.getItem(this.KEYS.AUTH)) {
            ...
            if (auth.profile_photo === undefined) { 
                auth.profile_photo = ''; updated = true; 
            }
    ...
    getSettings()          { return JSON.parse(localStorage.getItem(this.KEYS.SETTINGS)) || this.getDefaultSettings(); },
    ...
    saveCustomers(c)             { localStorage.setItem(this.KEYS.CUSTOMERS, JSON.stringify(c)); },
    ...
    saveInvoices(inv)      { localStorage.setItem(this.KEYS.INVOICES, JSON.stringify(inv)); },
    ...
    savePurchases(p)        { localStorage.setItem(this.KEYS.PURCHASES, JSON.stringify(p)); },
    ...
    saveSuppliers(s)        { localStorage.setItem(this.KEYS.SUPPLIERS, JSON.stringify(s)); },
    ...
    // ─── ITINERARIES ───
    getItineraries() {
        return JSON.parse(localStorage.getItem(this.KEYS.ITINERARIES)) || [];
    },
    saveItineraries(itineraries) {
        localStorage.setItem(this.KEYS.ITINERARIES, JSON.stringify(itineraries));
    },
    getActiveItineraries() {
        return this.getItineraries().filter(i => i.status !== 'deleted');
    },
    getItineraryById(id) {
        return this.getItineraries().find(i => i.id === id);
    },
    addItinerary(data) {
    ...
    deleteItinerary(id) {
        ...
        const idx = itineraries.findIndex(i => i.id === id);
        if (idx !== -1) {
            itineraries[idx].status     = 'deleted';
            itineraries[idx].deleted_at = new Date().toISOString();
            this.saveItineraries(itineraries);
            return true;
        }
        return false;
    ...
    getNextItinNumber() {
        const counter = parseInt(
            localStorage.getItem(this.KEYS.ITIN_COUNTER) || '0'
        ) + 1;
        const fy = getCurrentFY();
        return `TZ-IT-${fy}/${String(counter).padStart(3,'0')}`;
    },
    incrementItinCounter() {
        const counter = parseInt(
            localStorage.getItem(this.KEYS.ITIN_COUNTER) || '0'
        ) + 1;
        localStorage.setItem(this.KEYS.ITIN_COUNTER, counter.toString());
    },

    // ─── FY COUNTERS ───
    getFYCounters() {
        return JSON.parse(localStorage.getItem(this.KEYS.FY_COUNTER)) || {};
    },
    ...
    getNextInvoiceNumber() {
        ...
        return {
            invoiceNumber: `TZ/${fy}/${String(nextNum).padStart(3,'0')}`,
            financialYear: fy
        };
    },
    ...
    importAllData(data) {
        ...
        if (data.itineraries) this.saveItineraries(data.itineraries);
        if (data.fy_counters) {
            localStorage.setItem(
                this.KEYS.FY_COUNTER, JSON.stringify(data.fy_counters)
            );
        }
        if (data.auth) {
